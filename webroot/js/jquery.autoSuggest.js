...
(function($){
	$.fn.autoSuggest = function(data, options) {
		if(!$(this).parent().hasClass('as-original')){
			var defaults = { 
				asHtmlID: false,
						...
						for(var i=0;i<d_count;i++){				
							...
							if(forward){
								...
								if(opts.resultsHighlight){
									var replace_str=this_data[opts.selectedItemProp].toString();;
									this_data[opts.selectedItemProp] = replace_str.replace(regx,"<em>$1</em>");
								}
								if(!opts.formatList){
